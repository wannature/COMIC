...
class Causal_Norm_Classifier(nn.Module):
    ...
    def forward(self, x, label, embed):
        ...
        y_head_nomoving = y_b.clone()
        y_tail_nomoving = y_b.clone()
        ...
        #         y_head_temp = torch.mm((nx +  cos_val*0.5  * nc) * self.scale, nw.t())
        #         y_tail_temp = torch.mm((nx -  cos_val * self.alpha * nc) * self.scale, nw.t())
        #         output_tail.append(y_tail_temp)
        #         output_head.append(y_head_temp)
        #     y_tail_nomoving = sum(output_tail)
        #     y_head_nomoving = sum(output_head)
        return y_b, y_head_nomoving
