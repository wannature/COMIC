...
import pdb
import json
from loss.HeadTailBalancerLoss import HeadTailBalancerLoss
from loss.ReflectiveLabelCorrectorLoss import ReflectiveLabelCorrectorLoss
...
class MLTModel ():
    ...
    def batch_forward(self, images_raw,images_reverse,images_balance, labels=None, feature_ext=False, phase='train'):
        '''
        This is a general single batch running function. 
        '''

        # Calculate Features of balanced model
        self.balanced_features = self.balanced_networks['feat_model'](images_raw)
        if phase == 'train':
        ...
        self.head_features = self.head_networks['feat_model'](images_reverse)
        ...
        self.tail_features = self.tail_networks['feat_model'](images_balance)
        # update moving average
        if phase == 'train':
        ...
        # addtive attention
        #lzc self.keys=torch.cat([self.head_features.unsqueeze(1), self.tail_features.unsqueeze(1)],1)
        #lzc self.balanced_features=(self.balanced_features+ 0.1*self.balanced_networks['additive_attention'](self.balanced_features.unsqueeze(1),self.keys,self.keys).squeeze(1))
    ...
    def batch_loss(self, labels_raw,labels_reverse,labels_balance):
        ...
        # head loss
        self.head_loss = 0
        self.head_loss_perf = self.criterions['PerformanceLoss'](self.logits_head, labels_raw)
        self.head_loss_perf *=  self.criterion_weights['PerformanceLoss']
        self.head_loss += self.head_loss_perf
        self.head_loss +=self.rlc_head


        # tail loss
        self.tail_loss = 0
        self.tail_loss_perf = self.criterions['PerformanceLoss'](self.logits_tail, labels_reverse)
        self.tail_loss_perf *=  self.criterion_weights['PerformanceLoss']
        self.tail_loss += self.tail_loss_perf
        self.tail_loss +=self.rlc_tail

        # balanced loss
        self.balanced_loss=0
        #lzc: self.balanced_loss += self.htb_loss
        self.balanced_loss_perf = self.criterions['PerformanceLoss'](self.logits_balanced, labels_balance)
        self.balanced_loss_perf *=  self.criterion_weights['PerformanceLoss']   
        self.env_loss = [self.head_loss_perf,self.tail_loss_perf,self.balanced_loss_perf]
        self.irm_loss = torch.stack(self.env_loss).mean()  
        self.balanced_loss += self.irm_loss   
        self.balanced_loss+=self.rlc_balanced
    ...
    def get_ground_truth(self,indexes):
        ...
        paths_image=self.train_loader.dataset.get_image_path(np.array(indexes).reshape(1,-1)[0])###lzc ???
        ground_truth=torch.zeros((self.config["training_opt"]["batch_size"], self.config["training_opt"]["num_classes"]))###lzc ????
    ...
    def train(self):
        ...
        htb_loss = HeadTailBalancerLoss(gamma=self.training_opt['gamma'])
        htb_loss.PFM=self.criterions["PerformanceLoss"]
        rlc_loss_balanced=ReflectiveLabelCorrectorLoss(distribution_path=self.distribution_path)
        rlc_loss_head=ReflectiveLabelCorrectorLoss(distribution_path=self.distribution_path)
        rlc_loss_tail=ReflectiveLabelCorrectorLoss(distribution_path=self.distribution_path)
        # Loop over epochs
        for epoch in range(1, end_epoch + 1):
            ...
            #####lzc: for step, (inputs, labels, indexes) in enumerate(self.train_loader):
            for step ,(meta_origin,meta_reverse,meta_balance) in enumerate(self.train_loader):###lzc
                if step == self.epoch_steps:
                    break
                images_raw,images_reverse,images_balance = meta_origin['sample_image'].cuda()  , meta_reverse['sample_image'].cuda() , meta_balance['sample_image'].cuda()###lzc
                labels_raw,labels_reverse,labels_balance = meta_origin['sample_label'].cuda() , meta_reverse['sample_label'].cuda() , meta_balance['sample_label'].cuda()###lzc
                indexes_raw,indexes_reverse,indexes_balance = meta_origin['sample_index'].cuda() , meta_reverse['sample_index'].cuda() , meta_balance['sample_index'].cuda()###lzc
                #lzc：if self.do_shuffle:
                #lzc:       inputs, labels = self.shuffle_batch(inputs, labels)
                #lzc: inputs, labels = inputs.cuda(), labels.cuda()          
                ground_raw=self.get_ground_truth(indexes_raw)
                ground_reverse=self.get_ground_truth(indexes_reverse)
                ground_balance=self.get_ground_truth(indexes_balance)
            
                with torch.set_grad_enabled(True):    
                    self.batch_forward(images_raw,images_reverse,images_balance)###lzc
                    #lzc: self.htb_loss = htb_loss(self.logits_head_moving, self.logits_tail_moving, self.logits_balanced, labels)
        
                    self.rlc_head=0.2*rlc_loss_head(self.logits_head_moving, labels_raw, epoch, ground_raw)
                    self.rlc_tail=0.2*rlc_loss_tail(self.logits_tail_moving, labels_reverse, epoch, ground_reverse)
                    self.rlc_balanced=0.2*rlc_loss_balanced(self.logits_balanced, labels_balance, epoch, ground_balance)
                    self.batch_loss(labels_raw,labels_reverse,labels_balance)###lzc
    ...
    def eval(self,  epoch, phase='val', save_feat=False):
        ...
        for step ,(meta_origin,meta_reverse,meta_balance) in enumerate(self.train_loader):###lzc
            inputs, labels = meta_origin['sample_image'].cuda(), meta_origin['sample_label'].cuda()
    ...
    def save_model(self, epoch, best_epoch, best_model_weights, best_acc):
        ...
        model_dir = os.path.join(self.training_opt['log_dir'], 
                                 'final_model_balanced_voc_checkpoint.pth')
